...
function findCity(q: string | null) {
...
export async function GET(req: Request) {
  ...
  const city = findCity(cityQ) || CITIES[2]; // Default: Riyadh
  ...
  try {
    ...
    const cityName = lang === "ar" ? city.nameAr : city.nameEn;
    const text = lang === "ar" ? `${cityName} • الطقس غير متاح الآن` : `${cityName} • Weather unavailable`;
    return NextResponse.json({ ok: false, text, error: e?.message || "error" }, { status: 200 });
  }
}
