...
export async function POST(req: Request) {
  ...
  // 1) Check lock (DB) — إذا فشل الـ RPC نسمح بالمتابعة (وضع تطوير)
  {
    ...
    let okCreds = username === ADMIN_USERNAME && password === ADMIN_PASSWORD;
    if (!okCreds) {
      try {
        await db.query(`
        ...
        `);
        const res = await db.query("SELECT password_hash FROM admins WHERE username = $1 LIMIT 1", [username.toLowerCase()]);
        if (res.rows.length > 0) {
          const hash = String(res.rows[0].password_hash || "");
          if (hash) {
