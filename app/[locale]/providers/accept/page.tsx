 ...
 export default function ProviderAcceptPage({ params }: { params: { locale: string } }) {
  ...
  const [waitingMsg, setWaitingMsg] = React.useState("");
  const [pollTimer, setPollTimer] = React.useState<any>(null);
        ...
        try {
          ...
          if (j2 && j2.ok) {
            ...
            if (clientSecret) {
              ...
              setView("done");
              setWaitingPay(true);
              setWaitingMsg(isAr ? "بانتظار دفع العمولة ٢.٥٪..." : "Waiting for 2.5% commission...");
              if (pollTimer) {
                try { clearInterval(pollTimer); } catch {}
              }
              const t = setInterval(async () => {
                const id = intentionId || (typeof localStorage !== "undefined" ? localStorage.getItem("lk_last_intention") || "" : "");
                if (!id) return;
                try {
                  ...
                  if (vj && vj.ok) {
                    if (vj.paid) {
                      ...
                      setPollTimer(null);
                      setWaitingMsg(isAr ? "تم دفع العمولة بنجاح" : "Commission paid successfully");
                      setWaitingPay(false);
                    } else if (String(vj.status || "").toLowerCase() === "failed") {
                      try { clearInterval(t); } catch {}
                      setPollTimer(null);
                      setWaitingMsg(isAr ? "فشل دفع العمولة" : "Commission payment failed");
                      setWaitingPay(false);
                    }
                  }
                } catch {}
   ...
   return (
    <main className="lk-accept-wrap" style={{ padding: 16, display: "flex", justifyContent: "center", minHeight: "100vh", background: "#f9f9f9" }} dir={isAr ? "rtl" : "ltr"}>
      <div className="lk-accept-card" style={{ width: "100%", maxWidth: 500, background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 10px rgba(0,0,0,0.05)", height: "fit-content" }}>
         
         {view === "done" ? (
            <div style={{textAlign: 'center', padding: '40px 0'}}>
              <h2 style={{color: (waitingPay ? "#1f2937" : (msg.includes("رفض") ? "#d32f2f" : "#2e7d32"))}}>
                {waitingPay ? waitingMsg : msg}
              </h2>
            </div>
         ) : (
           <>
            ...
            {view === "accept" && (
              <div className="lk-accept-grid" style={{ display: "grid", gap: 12 }}>
                <div className="lk-two">
                 ...
                 {paymentMethod === "transfer" && (
                   <div style={{ background: "#f9f9f9", padding: 12, borderRadius: 8, display: "grid", gap: 10 }}>
                     ...
                     <div>
                       <label style={{ display: "block", fontSize: 12, fontWeight: 900, marginBottom: 4 }}>{isAr ? "رقم وثيقة العمل الحر" : "Freelance License No"}</label>
                       <input value={licenseNo} onChange={(e) => setLicenseNo(e.target.value)} placeholder="FL-276433011" style={{ width: "100%", height: 40, borderRadius: 8, border: "1px solid #ddd", padding: "0 10px" }} />
                     </div>
                   </div>
                 )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .lk-accept-card { margin-inline: auto; }
        @media (min-width: 768px) {
          .lk-accept-card { max-width: 640px; padding: 24px; }
          .lk-accept-grid { gap: 14px; }
          .lk-accept-grid .lk-two { display: grid; grid-template-columns: 1fr 180px; gap: 12px; align-items: end; }
        }
        @media (min-width: 1024px) {
          .lk-accept-card { max-width: 700px; }
          .lk-accept-grid .lk-two { grid-template-columns: 1fr 200px; }
        }
      ` }} />
     </main>
   );
