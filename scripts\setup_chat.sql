...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'conversations' AND policyname = 'deny_all_conversations') THEN
    DROP POLICY deny_all_conversations ON conversations;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'messages' AND policyname = 'deny_all_messages') THEN
    DROP POLICY deny_all_messages ON messages;
  END IF;
END$$;
CREATE POLICY deny_all_conversations ON conversations FOR ALL USING (false) WITH CHECK (false);
CREATE POLICY deny_all_messages ON messages FOR ALL USING (false) WITH CHECK (false);
